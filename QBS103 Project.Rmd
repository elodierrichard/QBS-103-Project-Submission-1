...
```{r}
###as.numeric for all the genes using
## rename the titles
## comment out things you don't need anymore

sub_2_plots <- function(data, genes, cat1 , cat2, cont ) {
  #filenames <- list.files(path = project1.data, pattern = ".csv")
    
    histogram2 <- ggplot(data, aes_string(x= genes)) + geom_histogram(bins = 20, color = 'navy', fill = 'lightblue') + labs(title = 'Gene Expression of AAAS', x= 'Gene: AAAS' , y= 'Frequency of AAAS' )
   ...
   boxplot2 <- ggplot(data, aes_string(x= cat1, y = genes, fill = cat2)) + geom_boxplot() + labs(title = 'Gene Expression of AAAS vs ICU Status and Mechanical Ventilation', x= 'ICU Status of Participant' , y= 'Gene Expression of AAAS', fill= 'Mechanical Ventilation')
...
specific_genes = subset(combined, select = c("AAAS", "AACS", "AADAC"))
for (gene in colnames(specific_genes)) {
  print(gene)
  print(combined[,gene])
